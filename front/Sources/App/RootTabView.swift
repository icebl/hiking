...
struct RootTabView: View {
    ...
    /// 全屏覆盖目标（地图 / 崩溃续记），合并为单一 item 避免多 cover 冲突。
    enum Cover: Identifiable {
        case map
        case resume(UUID)
        var id: String { switch self { case .map: return "map"; case .resume(let u): return "resume-\(u.uuidString)" } }
    }
    ...
    var body: some View {
        ...
        .onChange(of: selection) { new in
            if new == 1 { cover = .map; selection = 0 }   // 选中地图 → 全屏打开，Tab 复位
        }
